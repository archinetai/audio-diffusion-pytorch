...
class KerrasSchedule(SigmaSchedule):
    ...
    def __init__(self, sigma_min: float, sigma_max: float, rho: float = 7.):
        super().__init__() 
    ...
    def forward(self, num_steps: int, device: Any) -> Tensor:
        ...
        sigmas = F.pad(sigmas, pad=(0, 1), value=0.) 
        return sigmas 
...
class Diffusion(nn.Module):
    ...
    def denoise_fn(self, x_noisy: Tensor, sigmas: Tensor = None, sigma: float = None, clamp: bool = False) -> Tensor:
        batch, device = x_noisy.shape[0], x_noisy.device 

        assert exists(sigmas) or exists(sigma), "Either sigmas or sigma must be provided"
        ...
        x_denoised = self.c_skip(sigmas_padded) * x_noisy + self.c_out(sigmas_padded) * x_pred
        x_denoised = x_denoised.clamp(-1., 1) if clamp else x_denoised 
...
class DiffusionSampler(nn.Module):
    ...
    def step(self, x: Tensor, sigma: Tensor, sigma_next: Tensor, clamp: bool = True):
        """ Algorithm 2 (step) """ 
        gamma = self.gamma if self.s_tmin <= sigma <= self.s_tmax else 0. 
